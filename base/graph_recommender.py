...
class GraphRecommender(Recommender):
    ...
    def fast_evaluation(self, epoch):
        ...
        if len(self.bestPerformance) > 0:
            ...
            self.save()
        print('-' * 120)
        print('Real-Time Ranking Performance ' + ' (Top-' + str(' '.join(str(i) for i in self.topN)) + ' Item Recommendation)')
        measure = [m.strip() for m in measure[0:]]
        print('*Current Performance*')
        print('Epoch:', str(epoch + 1) + ',', '  |  '.join(measure))
        bp = ''
        for N in self.topN:
            bp += 'Recall@' + str(N) + ':' + str(self.bestPerformance[1]['Recall@' + str(N)]) + '  |  '
            bp += 'NDCG@' + str(N) + ':' + str(self.bestPerformance[1]['NDCG@' + str(N)]) + '  |  \n'
        print('*Best Performance* ')
        print('Epoch:', str(self.bestPerformance[0]) + ',\n', bp)
        print('-' * 120)
        return measure
