...
class Relation(Graph):
    def __init__(self, conf, relation, user):
        ...
        self.__initialize()
        self.social_num_dic = self.get_social_num_dic()
        self.social_norm_mat = self.normalize_graph_mat(self.get_social_mat())
    ...
    def get_social_num_dic(self):
        social_num_dic = dict.fromkeys(range(self.user_num), 0.0)
        for each_user in self.followees:
            social_num_dic[self.user[each_user]] = float(len(self.followees[each_user]))
        return social_num_dic
