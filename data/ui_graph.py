...
from data.data import Data,Rating
...
class Interaction(Data,Graph):
    def __init__(self, conf, training, test):
        ...
        self.interaction_num_dic = self.get_interaction_num_dic()
    ...
    def get_interaction_num_dic(self):
        interaction_num_dic = defaultdict(dict)
        interaction_num_dic['user'] = dict.fromkeys(range(self.user_num), 0.0)
        interaction_num_dic['item'] = dict.fromkeys(range(self.item_num), 0.0)
        for each_user in self.training_set_u:
            interaction_num_dic['user'][self.user[each_user]] = float(len(self.training_set_u[each_user]))
        for each_item in self.training_set_i:
            interaction_num_dic['item'][self.item[each_item]] = float(len(self.training_set_i[each_item]))
        return interaction_num_dic
...
class Rating_Graph(Rating,Graph):
    def __init__(self, conf, data, user_id_mapping, item_id_mapping, user_id_mapping2, item_id_mapping2):
        Graph.__init__(self)
        Rating.__init__(self, conf, data)

        self.user = user_id_mapping
        self.item = item_id_mapping
        self.id2user = user_id_mapping2
        self.id2item = item_id_mapping2
        self.training_set_u = defaultdict(dict)
        self.training_set_i = defaultdict(dict)
        self.__generate_set()
        self.user_num = len(user_id_mapping)
        self.item_num = len(item_id_mapping)
        self.ui_adj = self.__create_sparse_bipartite_adjacency()
        self.norm_adj = self.normalize_graph_mat(self.ui_adj)
        self.interaction_mat = self.__create_sparse_interaction_matrix()
        self.interaction_num_dic = self.get_interaction_num_dic()
    ...
    def __create_sparse_bipartite_adjacency(self, self_connection=False):
        ...
        '''
        n_nodes = self.user_num + self.item_num
        row_idx = [self.user[pair[0]] for pair in self.data]
        col_idx = [self.item[pair[1]] for pair in self.data]
        user_np = np.array(row_idx)
        item_np = np.array(col_idx)
        ratings = np.ones_like(user_np, dtype=np.float32)
        tmp_adj = sp.csr_matrix((ratings, (user_np, item_np + self.user_num)), shape=(n_nodes, n_nodes),dtype=np.float32)
    ...
    def data_size(self):
        return len(self.user), len(self.item), len(self.data)
