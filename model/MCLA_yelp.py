...
class MCLA_yelp(GraphRecommender):
    def __init__(self, conf, training_set, test_set, **kwargs):
        super(MCLA_yelp, self).__init__(conf, training_set, test_set, **kwargs)
        args = OptionConf(self.config['MCLA_yelp'])
        ...
        self.model = MCLA_yelp_Encoder(self.data, self.friend_data, self.group_data, self.emb_size, self.rating_emb_size,
                                  self.social_layers, self.rating_layers, self.eps_up, self.eps_down, self.emb_comb, self.rating_split_data)

    def print_model_info(self):
        super(MCLA_yelp, self).print_model_info()
...
class MCLA_yelp_Encoder(nn.Module):
    def __init__(self, data, friend_data, group_data, emb_size, rating_emb_size, social_layers, rating_layers, eps_up, eps_down, emb_comb, rating_split_data):
        super(MCLA_yelp_Encoder, self).__init__()
        self.data = data
        ...
        if os.path.isfile('noise\\yelp_noise_vectors_' + str(eps_down) + '-' + str(eps_up) + '.pickle'):
            print('用户活跃度文件存在')
            with open('noise\\yelp_noise_vectors_' + str(eps_down) + '-' + str(eps_up) + '.pickle', 'rb') as f:
            ...
            with open('noise\\yelp_noise_vectors_' + str(eps_down) + '-' + str(eps_up) + '.pickle', 'wb') as f:
