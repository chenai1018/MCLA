...
class MCLA_Epinions(GraphRecommender):
    def __init__(self, conf, training_set, test_set, **kwargs):
        super(MCLA_Epinions, self).__init__(conf, training_set, test_set, **kwargs)
        args = OptionConf(self.config['MCLA_Epinions'])
        ...
        for each_rating in kwargs['rating.split']:
            self.rating_split_data[int(each_rating)] = Rating_Graph(conf, kwargs['rating.split'][each_rating],
                                                                    self.data.user, self.data.item, self.data.id2user,
                                                                    self.data.id2item)
        self.friend_data = Relation(conf, kwargs['friend.data'], self.data.user)
        self.model = MCLA_Epinions_Encoder(self.data, self.friend_data, self.emb_size, self.rating_emb_size,
                                  self.social_layers, self.rating_layers, self.eps_up, self.eps_down, self.emb_comb,
                                  self.rating_split_data)

    def print_model_info(self):
        super(MCLA_Epinions, self).print_model_info()
        print('Trust data size: (user number: %d, relation number: %d).' % (self.friend_data.size()))
        print('=' * 80)
...
def min_max_range(x, range_values):
    return [round(((xx - min(x)) / (1.0 * (max(x) - min(x)))) * (range_values[1] - range_values[0]) + range_values[0], 4) for xx in x]


class MCLA_Epinions_Encoder(nn.Module):
    def __init__(self, data, friend_data, emb_size, rating_emb_size, social_layers, rating_layers, eps_up, eps_down, emb_comb, rating_split_data):
        super(MCLA_Epinions_Encoder, self).__init__()
        self.data = data
        ...
        print("获取交互数量...")
        for each_rating in tqdm(rating_split_data):
        ...
        if os.path.isfile('noise\\epinions_noise_vectors_' + str(eps_down) + '-' + str(eps_up) + '.pickle'):
            print('用户活跃度文件存在')
            with open('noise\\epinions_noise_vectors_' + str(eps_down) + '-' + str(eps_up) + '.pickle', 'rb') as f:
                self.rating_noise_vectors = pickle.load(f)
        else:
            print('用户活跃度文件不存在，正在计算......')
            for each_rating in tqdm(self.rating_interaction_num):
            ...
            with open('noise\\epinions_noise_vectors_' + str(eps_down) + '-' + str(eps_up) + '.pickle', 'wb') as f:
    ...
    def _init_model(self):
        ...
        return init_emb_SGU_dic, rating_embedding_dict, social_embedding_dict
    ...
    def cal_att_scores_and_embs(self, embeddings_dic, type):
        ...
        for rating in sorted(embeddings_dic):
            weights.append(torch.tanh(torch.sum(
                self.init_emb_SGU[str(type) + '_att'](self.init_emb_SGU[str(type) + '_att_mat'](embeddings_dic[rating])), dim=1)))
